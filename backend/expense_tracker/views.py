...
def add_income(request):
	...
	# Save to database
	income = Income.objects.create(amount=amount, date=date_obj.date(), user=user)
...
def get_expenses(request, date_start=None, date_end=None):
	...
	expense = Expense.objects.prefetch_related('tags').filter(user=user, date__range=(date_start, date_end)).order_by('-date')
