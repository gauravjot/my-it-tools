...
export default function JobApplicationTrackerTool() {
	const [jobApps, setJobApps] = useLocalStorage<z.infer<typeof schema>[]>(
		"job_tracker_applications",
		[]
	);
	const [statusFilter, setStatusFilter] = useLocalStorage<z.infer<typeof schema>["status"] | null>(
		"job_tracker_status_filter",
		null
	);
	...
	return (
		<BaseSidebarLayout title="Job Application Tracker">
			<div className="m-4 lg:m-8 flex flex-col lg:flex-row gap-2">
				<h1 className="text-2xl flex-1 font-bold tracking-tight leading-7">
							...
							<DropdownMenuGroup>
								...
								{STATUSES.map((status) => (
									<DropdownMenuItem
										className="py-1"
										onClick={() => {
											setStatusFilter(status as z.infer<typeof schema>["status"]);
										}}
									>
										<StatusBadge status={status as z.infer<typeof schema>["status"]} />
									</DropdownMenuItem>
								))}
							</DropdownMenuGroup>
						</DropdownMenuContent>
			...
			<div className="flex flex-col xl:flex-row m-4 lg:m-8 gap-4 max-w-[2000px]">
								...
								.map((application) => (
									<TableRow key={application.role + application.company + application.date}>
										<TableCell className="font-medium min-w-[200px]">
											{application.role} &mdash; {application.company}
										</TableCell>
													...
													<DropdownMenuGroup>
														{STATUSES.map((status) => (
															<DropdownMenuItem
																className="py-1"
																onClick={() => {
																	const newArr = jobApps.slice();
																	const index = newArr.findIndex(
																		(a) =>
																			a.role + a.company + a.date ===
																			application.role + application.company + application.date
																	);
																	const row = newArr.splice(index, 1)[0];
																	row.status = status as z.infer<typeof schema>["status"];
																	setJobApps([
										...
										<TableCell className="text-sm text-muted-foreground">
											{dateTimePretty(application.date)}
										</TableCell>
										<TableCell className="flex place-items-center w-full gap-4">
											<div className="flex-1 text-sm text-muted-foreground">
												{application.application_detail_url &&
												application.application_detail_url.length > 0 ? (
													<a href={application.application_detail_url} target="_blank">
														Application Link
													</a>
												) : (
													<></>
												...
												{application.portal_url && application.portal_url.length > 0 ? (
													<>
														{" • "}
														<a href={application.portal_url} target="_blank">
															Portal Link
														</a>
													</>
												) : (
				...
				<div className="flex order-1 xl:order-2 w-full xl:w-80 mx-auto place-items-center">
					<div className="w-full p-4 lg:p-6 bg-white shadow-md dark:bg-zinc-900 rounded-xl">
						...
						<Form {...form}>
							<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3 mt-4">
								...
								<Button variant="default" type="submit" className="flex-1 flex gap-2 w-full">
